...
export function CardTitle({
  ...
  return <h3 className={cn("text-lg font-bold leading-snug hx-text-title", className)}>{children}</h3>;
}
...
export function CardDescription({
  ...
  return <p className={cn("mt-2 text-sm leading-6 hx-text-muted", className)}>{children}</p>;
}
...
export function CardFooter({
  ...
  return (
    <div
      className={cn(
        "mt-5 flex flex-wrap items-center gap-4 border-t border-[hsl(var(--sidebar-border))] pt-4",
        className,
      )}
    >
      {children}
