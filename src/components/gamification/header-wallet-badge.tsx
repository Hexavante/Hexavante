...
export async function HeaderWalletBadge({ userId }: Props) {
  ...
  return (
    <div className="hidden items-center gap-2 sm:flex">
      ...
      />
      <Link href="/shop" className="hx-header-wallet" title="Suas moedas">
        <Coins className="h-4 w-4 shrink-0 hx-header-wallet-icon" />
        <span className="tabular-nums">{profile.coins.toLocaleString("pt-BR")}</span>
      </Link>
    </div>
  );
}
