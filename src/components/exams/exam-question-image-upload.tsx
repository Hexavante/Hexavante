...
export const ExamQuestionImageUpload = forwardRef<ExamQuestionImageUploadHandle, Props>(
  function ExamQuestionImageUpload(
    { initialUrl, initialWidth, initialHeight, initialDisplaySize, name = "imageUrl" },
    ref,
  ) {
          ...
          {hasPreview ? (
            hasDimensions ? (
              ...
              <div className="flex min-h-28 flex-col items-center justify-center gap-2">
                {/* img for local blob preview URL — next/image does not support blob: URLs */}
        ...
        {hasPreview && hasDimensions && (
          <div className="max-w-xs">
            ...
            <NativeSelect
              id="imageDisplaySize"
              value={displaySize}
              onChange={(event) => setDisplaySize(event.target.value as ExamQuestionImageSize)}
            >
