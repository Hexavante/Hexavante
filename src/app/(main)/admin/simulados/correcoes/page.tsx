...
export default async function EssayCorrectionsPage() {
  ...
  if (!session?.user?.id) redirect("/login?callbackUrl=/admin/simulados/correcoes");
  ...
  return (
    <PageShell size="lg">
      <AppLink href="/admin/simulados" muted className="mb-4 inline-flex items-center gap-1">
        ← Simulados
      </AppLink>
