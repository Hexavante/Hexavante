...
export default async function ModerationExamsPage() {
  ...
  if (!session?.user?.id) redirect("/login?callbackUrl=/admin/simulados");
  ...
  return (
    <PageShell>
      <AppLink href="/admin" muted className="mb-4 inline-flex items-center gap-1">
        ← Moderação
      </AppLink>
      ...
      <PageHeader
        ...
        action={
          <div className="flex flex-wrap gap-2">
            <LinkButton href="/admin/simulados/correcoes" variant="outline" size="sm">
              Correções dissertativas
            </LinkButton>
            <LinkButton href="/admin/simulados/new" size="sm">
              Novo simulado
            </LinkButton>
          </div>
        }
      ...
      {exams.length === 0 ? (
        <EmptyState icon={ClipboardList} title="Nenhum simulado cadastrado.">
          <LinkButton href="/admin/simulados/new" className="mt-4">
