...
export async function reportCommunityPostAction(
  activityId: string,
  reason: string,
  details?: string,
): Promise<CommunityActionResult> {
  const session = await auth();
  if (!session?.user?.id) {
    return { success: false, error: "Faça login para denunciar." };
  }

  const parsed = reportCommunityPostSchema.safeParse({ activityId, reason, details });
  if (!parsed.success) {
    return { success: false, error: parsed.error.issues[0]?.message ?? "Dados inválidos." };
  }
