...
const RECENT_FRAGMENT_WINDOW_MS = 10 * 60 * 1000;
const MAX_RECENT_FRAGMENTS = 8;

async function getRecentCommunityFragments(
  userId: string,
  activityId: string,
): Promise<string[]> {
  const since = new Date(Date.now() - RECENT_FRAGMENT_WINDOW_MS);
...
export async function addActivityComment(
  ...
  if (activity.userId !== userId) {
    ...
    const safePreview = filterProfanity(content);
    await createNotification({
      userId: activity.userId,
      type: "COMMUNITY_REPLY",
      title: "Nova resposta na comunidade",
      message: `${authorName}: ${safePreview.slice(0, 100)}${safePreview.length > 100 ? "…" : ""}`,
